...
def test(paths):
 ...
 'checkpoints/ViT-L-14-336_openai_2_0.992174262897354.pth'
    ...
    probs = torch.from_numpy(np.array(list(path2pred.values()))).softmax(1)
    result_df['filename'] = path2pred.keys()
    result_df.loc[:, ['шипун', 'кликун', 'малый']] = probs

    return result_df.to_dict("records")


if __name__=='__main__':
    result = test('/media/ivan/Data1/hack_ai/Тестовый датасет/')
    print(result)
